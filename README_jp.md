...
- `/random-read` は `words`（ワードデバイスのアドレス文字列配列）と `dwords`（同じくダブルワード読み取り用）を受け付けます。どちらかが非空であれば可。ワードデバイス（`D`、`W`、`R` など）のみ対応。レスポンスは `{"words":[...],"dwords":[...]}` で、word 値は整数（`0..65535`）、dword 値は符号なし 64 ビット整数です。
...
ワードデバイスのアドレスに `.N`（16進1桁、`0`〜`F`）を付けると、1 ビットのみを読み書きできます。
