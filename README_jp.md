...
ビットデバイスのアドレスに `K<n>`（`n` は `1`〜`8`）を前置すると、連続する `n * 4` 個の
ビットデバイスを 1 つの整数として扱えます。三菱ラダーの `K` 記法と同じ考え方です。
ビットは下位側から詰められ、値の第 `i` ビットが `addr + i` のデバイスに対応します。

```
K1M100   ← M100〜M103 を  4 ビット値（0〜15）として扱う
K2M100   ← M100〜M107 を  8 ビット値（0〜255）として扱う
K4M100   ← M100〜M115 を 16 ビット値（0〜65535）として扱う
K8M100   ← M100〜M131 を 32 ビット値として扱う
```

- `GET /read?addr=K4M100` は `{"values":[43981]}` を返します（常に要素 1 個の配列）。
- `POST /write?addr=K4M100` に `{"values":[43981]}` を渡すと `M100`〜`M115` の 16 ビットを書き込みます。
- `sint=true` は K のビット幅で符号拡張します。`K4M100&sint=true` は `-32768`〜`32767`、`K2M100&sint=true` は `-128`〜`127` です。書き込み時に範囲外の値は `400 bad_request` になります（例: `K1` に `16`、`sint` なしで `-1`）。
- ビットデバイス専用です。`K4D100`（ワードデバイス）は `400 bad_request` になります。
- `count` は非対応です。1 リクエストで扱えるのは 1 値のみです。16 ビットを超える値は `dword=true`（拒否されます）ではなく `K5`〜`K8` を使ってください。
- `K0`、`K9` 以上は `400 bad_request` です。ビットアクセスとの併用（`K4M100.1`）も同様です。
- K記法が使えるのは `/read` と `/write` のみです。`/random-read`・`/random-write` では、基準デバイスとして黙って読み書きされることを防ぐため `400 bad_request` になります。

> **性能上の注意:** K記法の読み取りは `ReadBits`（0x0401）1 回、書き込みは `WriteBits` 1 回で完結します。ワードデバイスのビットアクセスと違い read-modify-write のコストはかかりません。
